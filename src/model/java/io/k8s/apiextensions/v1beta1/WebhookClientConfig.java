...
public class WebhookClientConfig {
     ...
     * field instead. The host might be resolved via external DNS in some apiservers (e.g., `kube-apiserver` cannot
     * resolve in-cluster DNS as that would be a layering violation). `host` may also be an IP address.
     * 
     * Please note
     * that using `localhost` or `127.0.0.1` as a `host` is risky unless you take great care to run this webhook on all
     * hosts which run an apiserver which might need to make calls to this webhook. Such installs are likely to be
