...
public class ServiceSpec {
     ...
     * with this IP.  A common example is external load-balancers that are not part of the Kubernetes system.
     */
    @Nullable
    @Singular
     ...
     * No proxying will be involved. Must be a valid RFC-1123 hostname (https://tools.ietf.org/html/rfc1123) and
     ...
     * specified by the client. Only effects when Type is set to LoadBalancer and ExternalTrafficPolicy is set to Local.
     */
    @Nullable
     ...
     * IPv6DualStack feature gate is enabled. In a dual-stack cluster, you can specify ipFamily when creating a
     * ClusterIP Service to determine whether the controller will allocate an IPv4 or IPv6 IP for it, and you can
     * specify ipFamily when creating a headless Service to determine whether it will have IPv4 or IPv6 Endpoints. In
     * either case, if you do not specify an ipFamily explicitly, it will default to the cluster's primary IP family.
     * This field is part of an alpha feature, and you should not make any assumptions about its semantics other than
     * those described above. In particular, you should not assume that it can (or cannot) be changed after creation
     * time; that it can only have the values "IPv4" and "IPv6"; or that its current value on a given Service correctly
     * reflects the current state of that Service. (For ClusterIP Services, look at clusterIP to see if the Service is
     * IPv4 or IPv6. For headless Services, look at the endpoints, which may be dual-stack in the future. For
     * ExternalName Services, ipFamily has no meaning, but it may be set to an irrelevant value anyway.)
     */
    @Nullable
    String ipFamily;
    ...
    /**
     * publishNotReadyAddresses indicates that any agent which deals with endpoints for this Service should disregard
     * any indications of ready/not-ready. The primary use case for setting this field is for a StatefulSet's Headless
     * Service to propagate SRV DNS records for its Pods for the purpose of peer discovery. The Kubernetes controllers
     * that generate Endpoints and EndpointSlice resources for Services interpret this to mean that all endpoints are
     * considered "ready" even if the Pods themselves are not. Agents which consume only Kubernetes generated endpoints
    ...
    /**
     * topologyKeys is a preference-order list of topology keys which implementations of services should use to
     * preferentially sort endpoints when accessing this Service, it can not be used at the same time as
     * externalTrafficPolicy=Local. Topology keys must be valid label keys and at most 16 keys may be specified.
     * Endpoints are chosen based on the first topology key with available backends. If this field is specified and all
     * entries have no backends that match the topology of the client, the service has no backends for that client and
     * connections should fail. The special value "*" may be used to mean "any topology". This catch-all value, if used,
     * only makes sense as the last value in the list. If this is not specified or empty, no topology constraints will
     * be applied.
     */
    @Nullable
    @Singular
     ...
     * ClusterIP and allocates a port on every node which routes to the clusterIP. "LoadBalancer" builds on NodePort and
     * creates an external load-balancer (if supported in the current cloud) which routes to the clusterIP. More info:
