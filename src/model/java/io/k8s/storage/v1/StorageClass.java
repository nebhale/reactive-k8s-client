...
import io.k8s.core.v1.TopologySelectorTerm;
...
/**
 * StorageClass describes the parameters for a class of storage for which PersistentVolumes can be dynamically
 * provisioned.
 * 
...
@Jacksonized
@Value
public class StorageClass implements ObjectMeta, TypeMeta {
    ...
    /**
     * Restrict the node topologies where volumes can be dynamically provisioned. Each volume plugin defines its own
     * supported topology specifications. An empty TopologySelectorTerm list means there is no topology restriction.
