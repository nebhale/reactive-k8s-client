...
@AllArgsConstructor
@ToString
public final class InterfaceGenerator implements Generator {
    ...
    public void writeTo(Path root) throws IOException {
        Path path = root.resolve(Paths.get(String.format("%s/%s.java", _interface.getPackage().replaceAll("\\.", File.separator), _interface.getName())));
        ...
        try (Writer writer = Files.newBufferedWriter(path, StandardOpenOption.CREATE_NEW, StandardOpenOption.WRITE)) {
            mustacheFactory.compile("interface.mustache").execute(writer, _interface);
        }
    }
